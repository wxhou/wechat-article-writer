...
description: 公众号文章创作与发布。完整流程：自动场景识别 → 获取内容 → 质量检查 → 写作/翻译 → AI配图 → 封面图 → 格式优化 → 发布。使用场景：(1) X/网页链接 (2) X Article 长文 (3) 英文文章翻译 (4) 标题创作 (5) 直接发布
---
...
| **X Article 长文** | 包含 /status/ + 内容较长 | 获取内容 → 深度改写 → 配图 → 封面 → 发布 |
| 英文文章 | 纯英文、English | 翻译 → 写作 → 配图 → 封面 → 发布 |
| 仅有标题 | 无链接、纯中文标题 | article-writer 调研 → 写作 → 配图 → 封面 → 发布 |
| 直接发布 | "直接发布"、"走完整流程" | 跳过草稿，直接完整流程 |

**X Article 特点**：
- 内容深度足够，可直接做素材
- 需用 Jina Reader 获取：`https://r.jina.ai/<链接>`
- 改写时保留核心观点，融入个人风格
